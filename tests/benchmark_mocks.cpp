...
void BM_crtp_gmock(benchmark::State& state) {
    ...
    EXPECT_CALL(s.serverInterface(), startServer()).WillRepeatedly(::testing::Return(true));
    EXPECT_CALL(s.databaseInterface(), initConnection()).WillRepeatedly(::testing::Return(true));
